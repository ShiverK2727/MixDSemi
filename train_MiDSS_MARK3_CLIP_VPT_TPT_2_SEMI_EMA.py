...
parser.add_argument('--lambda_ce', type=float, default=1.0,
                    help='(新) Weight for supervised Cross-Entropy loss (L_ce)')
parser.add_argument('--lambda_dice', type=float, default=1.0,
                    help='(新) Weight for supervised Dice loss (L_dice)')
...
def train(args, snapshot_path):
    ...
    for iter_num in p_bar:
        ...
        with amp_cm():
            ...
            # === Teacher 前向 (不接收梯度) ===
            with torch.no_grad():
                # 4. Unlabeled (weak aug) -> L_consis 的伪标签
                ...
                ulb_weak_outputs = model_teacher(ulb_images_weak_preprocessed, args.class_text)
                        ...
                        # ========== (新) Loss 计算 (v6 / CE + Dice + FixMatch) ==========
            
            loss_total = torch.tensor(0.0, device=device)
            loss_ce_w = torch.tensor(0.0, device=device) # (新)
            loss_ce_s = torch.tensor(0.0, device=device) # (新)
            loss_dice_w = torch.tensor(0.0, device=device) # (新)
            loss_dice_s = torch.tensor(0.0, device=device) # (新)
            ...
            # 1. 监督损失 (L_sup) - [Labeled Data] (新) CE + Dice
            if 'unet_label_14' in lb_sample and (args.lambda_ce > 0 or args.lambda_dice > 0):
                ...
                # (新) CE Loss (KL Div on hard labels)
                if args.lambda_ce > 0:
        ...
        writer.add_scalar('train/loss_ce_w', loss_ce_w.item(), iter_num) # (新)
        writer.add_scalar('train/loss_ce_s', loss_ce_s.item(), iter_num) # (新)
        writer.add_scalar('train/loss_dice_w', loss_dice_w.item(), iter_num) # (新)
        writer.add_scalar('train/loss_dice_s', loss_dice_s.item(), iter_num) # (新)
