    ...
    for eval_cycle in range(num_eval_cycles):
        ...
        for i_batch in range(1, args.num_eval_iter+1):
            ...
            try:
                ...
                # loss_distill may be a tensor-like or zero scalar
                try:
                    writer.add_scalar('train/loss_distill', float(loss_distill), iter_num)
                except Exception:
                    writer.add_scalar('train/loss_distill', 0.0, iter_num)

            # Try to obtain invariant/specific contributions (if the helper exists and distillation is active)
            distill_inv_val = 0.0
            distill_spec_val = 0.0
            if distill_active:
                try:
                    # Reuse available student/teacher features in scope to compute components safely
                    comps = losses.compute_distillation_components(
                        lb_u_inv=locals().get('lb_u_inv', None),
                        lb_u_spec=locals().get('lb_u_spec', None),
                        ulb_u_inv_s=locals().get('ulb_u_inv', None),
                        ulb_u_spec_s=locals().get('ulb_u_spec', None),
                        v_inv_lb_teacher=locals().get('v_inv_lb_teacher', None),
                        v_spec_lb_teacher=locals().get('v_spec_lb_teacher', None),
                        v_inv_ulb_teacher_w=locals().get('v_inv_ulb_teacher_w', None),
                        v_spec_ulb_teacher_w=locals().get('v_spec_ulb_teacher_w', None),
                        mode=args.distill_mode,
                    )
                    # components may be tensors or None
                    if getattr(comps, 'invariant', None) is not None:
                        distill_inv_val = float(comps.invariant.detach()) if hasattr(comps.invariant, 'detach') else float(comps.invariant)
                    if getattr(comps, 'specific', None) is not None:
                        distill_spec_val = float(comps.specific.detach()) if hasattr(comps.specific, 'detach') else float(comps.specific)
                except Exception:
                    # Fallback: if we can't compute split, keep zeros
                    distill_inv_val = 0.0
                    distill_spec_val = 0.0

            writer.add_scalar('train/distill_invariant', distill_inv_val, iter_num)
            writer.add_scalar('train/distill_specific', distill_spec_val, iter_num)
            ...
            if iter_num % 50 == 0:
                try:
                    logging.info(
                        "iter %d: loss=%.6f, L=%.6f, UL=%.6f, LU=%.6f, S=%.6f, distill=%.6f, inv=%.6f, spec=%.6f, clip_mv=%.6f, clip_ortho=%.6f, clip_sw=%.6f, cons_w=%.6f",
                        iter_num,
                        ...
                        float(distill_inv_val),
                        float(distill_spec_val),
                    ...
                    # Don't let logging failures stop training
                    pass
