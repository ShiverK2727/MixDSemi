...
keyword_test_groups = {
    # 目标关键词
    'Keyword_A_photo_of_prostate': ["A photo of prostate"],
    'Keyword_A_photo_of_prostate_gland': ["A photo of prostate gland"],

    # 风格/模态关键词
    'Keyword_MRI': ["MRI"],
    'Keyword_T2_weighted_MRI': ["T2-weighted MRI"],
    'Keyword_axial_plane': ["axial plane"],

    # 对照组
    'Keyword_medical_image': ["medical image"],
    'Keyword_human_body': ["human body"],
    'Keyword_background': ["background"]
}

# --- 输出目录 ---
OUTPUT_DIR = "/app/MixDSemi/SynFoCLIP/code/clip_roi_results_multi"
...
def main():
    ...
    for img_idx, img_path in enumerate(all_images):
        try:
            ...
            # 为每个文本组生成ROI
            for group_name, text_anchor in text_anchors.items():
                print(f"    Generating ROI for '{group_name}'")
                
                # 计算 Patch 与 当前文本锚点 的相似度
                # text_anchor 形状: (1, 512) -> text_anchor.t() 形状: (512, 1)
                # (1, 196, 512) @ (512, 1) -> (1, 196, 1)
                roi_logits = patch_features @ text_anchor.t()
                ...
                # g. 保存ROI结果（为每个文本组分别保存）
                group_clean_name = group_name.replace(':', '').replace('"', '').replace(' ', '_')
                
                # 保存为 .npy (用于后续的模型训练)
                npy_path = os.path.join(sample_dir, f"roi_heatmap_{group_clean_name}.npy")
                np.save(npy_path, roi_upsampled.astype(np.float32))

                # 保存为热力图 (用于可视化检查)
                heatmap_path = os.path.join(sample_dir, f"roi_heatmap_{group_clean_name}.png")
                plt.imsave(heatmap_path, roi_upsampled, cmap='viridis')
                
                # (可选) 保存叠加图
                overlay_path = os.path.join(sample_dir, f"roi_overlay_{group_clean_name}.png")
