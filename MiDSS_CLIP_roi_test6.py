...
def generate_heatmap_sw_cam(model, preprocess, strong_aug_transform, image_pil, prompt_group, device):
    ...
    # 3.3 计算 "不稳定性" (Instability)
    # 相似度范围 [-1, 1]。
    # (1.0 - similarity) 范围 [0, 2]。
    # 0 = 稳定 (高相似度)
    # 2 = 极不稳定 (反向)
    # 我们将 "不稳定性" 作为热图的分数。
    instability = 1.0 - similarity 
    
    instability_map = instability.squeeze(0).detach() # [N]
    
    # 此时，高分 (接近2.0) = 不稳定 = 假设的ROI
    # 低分 (接近0.0) = 稳定 = 假设的背景
    
    # --- 步骤 4: Reshape 和上采样 ---
    
    num_patches = instability_map.shape[0]
    ...
    heatmap_low_res = instability_map.reshape(1, 1, num_patches_side, num_patches_side)
...
def visualize_heatmaps(original_image, heatmap_inv, heatmap_spec):
    ...
    fig.suptitle(f"Heatmap based on Strong-Weak *Instability* (SW-CAM)", fontsize=16) # 标题更新
    plt.tight_layout()
