...
gap_group.add_argument('--gap_encoder_only', action='store_true',
                       help='仅对编码器参数应用 GapMatch 扰动（解码器保持原样）')
gap_group.add_argument('--gap_offload_cpu', action='store_true',
                       help='将 GapMatch 备份和中间梯度存放在 CPU 以降低 GPU 显存占用')
...
def train(args, snapshot_path):
    ...
    if args.enable_gapmatch:
        ...
        encoder_params = _collect_module_params(encoder_modules)
        decoder_params = _collect_module_params(decoder_modules)

        if distill_active:
            # 蒸馏模式下需要同步扰动风格/不变特征头
            encoder_params.extend(_collect_module_params(head_modules))
        else:
            # 否则头部通常参与解码分支
            decoder_params.extend(_collect_module_params(head_modules))

        encoder_params = _deduplicate_params(encoder_params)
        decoder_params = _deduplicate_params(decoder_params)

        encoder_param_count = len(encoder_params)
        decoder_param_count = len(decoder_params)

        if args.gap_encoder_only:
            decoder_params = []
            logging.info(
                "[GapMatch] 仅对编码器及相关头部施加扰动（编码器参数数=%d）。",
                encoder_param_count,
            )
        else:
            logging.info(
                "[GapMatch] 同时扰动编码器和解码器（编码器参数数=%d，解码器参数数=%d）。",
                encoder_param_count,
                decoder_param_count,
            )

        if args.gap_offload_cpu:
            logging.info("[GapMatch] 启用 CPU offload，备份和梯度缓存将移至 CPU 降低显存峰值。")

        gap_perturber = gapmatch_targeted_v1(
            encoder_params=encoder_params,
            decoder_params=decoder_params,
            gamma=args.gap_gamma,
            offload_to_cpu=args.gap_offload_cpu,
        )
