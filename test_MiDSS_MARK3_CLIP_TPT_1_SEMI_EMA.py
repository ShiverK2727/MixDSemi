...
from biomedclip_vpt_tpt_seg import build_tpt_seg_model, preprocess_tensor_images
...
def parse_args():
	parser = argparse.ArgumentParser(
		description="Run TPT-only (EMA) inference on the test set and visualize predictions."
	)
	...
	# visual_num_prompts removed: this test is for TPT-only models
...
def main():
	...
	# 实例化 VPT+TPT 模型
	print(f"Loading TPT-only model from {args.biomedclip_path}...")
	model, preprocess, tokenizer = build_tpt_seg_model(
	...
	# 加载训练好的 prompts（兼容 load_prompts 或 load_all_prompts）
	...
	try:
		...
		try:
			...
			# 最后尝试直接 load_state_dict（如果文件存的是 state dict）
			try:
				state = torch.load(args.prompts_path, map_location='cpu')
				if isinstance(state, dict):
					# 如果包含 prompts 保存的特定键，尝试呼叫对应加载
					if 'prompts' in state and hasattr(model, 'load_prompts_from_state'):
						model.load_prompts_from_state(state)
					else:
						# 尝试 model.load_state_dict
						model.load_state_dict(state)
				else:
					raise RuntimeError('Unsupported prompts file format')
			except Exception as e:
				raise RuntimeError(f'Failed to load prompts from {args.prompts_path}: {e}')
	model.eval()
